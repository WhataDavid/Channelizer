...
def plot_sub_and_cut(data, CHANNEL_NUM, D, title, cut=True):
    plt.figure(figsize=(12, 16), dpi=400)
    sub_bangwidth = 800 // CHANNEL_NUM
    ...
    for i in range(CHANNEL_NUM):
        ...
        # 计算该通道数据的傅里叶变换
        fft_data = np.abs(np.fft.fftshift(np.fft.fft(data[i], axis=0)))
        print("fft_data1:", fft_data.shape)
        fft_data = cut_extra_channel_data_by_side(fft_data, M, D)
        print("fft_data2:", fft_data.shape)
...
# file_path = r'csv\iladata_2507232002_dellete0_4tap.txt' #删除0后的数据
# file_path = r'csv\iladata_2507232328_circular_shift.txt' #循环移位后的数据
# file_path = r'csv\iladata_change_shift_direction_4tap_real.txt' #反转循环移位后的数据
file_path = r'csv\iladata_2507271817_dft.txt'
file_path = r'csv\260507_pfb.txt'
# 测试数据：
# file_path = r'csv\iladata_250717171826.txt'
# file_path = r'csv\iladata_2507272007_01234567_fakedata.txt'
...
print(np.fft.fft([1, 1, 1, 1, 1, 1, 1, 1]))
print(np.fft.fft([2, 2, 2, 2, 2, 2, 2, 2]))
# sys.exit()
...
print("reshape_data:\n", reshape_data.shape, "\n", reshape_data)
# np.savetxt("csv\\reshape_data.txt", reshape_data, delimiter=",")
flip_data = np.flip(reshape_data, axis=1)
print("flip_data:\n", flip_data.shape, "\n", flip_data)
# np.savetxt("csv\\flip_data.txt", flip_data, delimiter=",")
real_data = flip_data[:, 0:8]
print("real_data:\n", real_data.shape, "\n", real_data)
imag_data = flip_data[:, 8:16]
print("imag_data:\n", imag_data.shape, "\n", imag_data)
for i in flip_data[0]:
    print(i, end=",")
print("\n")
for i in real_data[0]:
    print(i, end=",")
print("\n")
for i in imag_data[0]:
    print(i, end=",")
print("\n")

complex_data = real_data + 1j * imag_data
print("complex_data:\n", complex_data.shape, "\n", complex_data)
np.savetxt("csv\\complex_data.txt", complex_data, delimiter=",", fmt='%.6f')

transpose_data = np.flip(real_data.T + 1j * imag_data.T, axis=1)
print("transpose_data:\n", transpose_data.shape, "\n", transpose_data)
np.savetxt("csv\\transpose_data.txt", transpose_data, delimiter=",", fmt='%.6f')

# dx_ospfb_fft = np.fft.ifft(transpose_data, axis=0)
plot_sub(transpose_data, M, D, "DX " + str(M) + "/" + str(D) + "X ospfb with z gcd and rotate result:", cut=False)
plot_sub_and_cut(transpose_data, M, D, "DX " + str(M) + "/" + str(D) + "X ospfb with z gcd and rotate result:",
                 cut=False)
